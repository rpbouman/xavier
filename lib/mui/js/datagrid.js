...
(DataGrid = function(conf) {
  ...
  setRows: function(dataRows) {
    ...
    this.deleteTableRows(rowsDom);
    var n = this.rowHeaders ? this.rowHeaders.length : 0;
    if (n) {
      this.addPositionableRow(rowsDom, this.rowHeaders.length);
      this.rows = dataRows;
      this.eachRow(function(i, row){
        this.createRowHeaders(i, row);
      }, this);
    }
  },
  ...
  setCells: function(dataCells) {
    var cellsDom = this.getCellsTableDom();
    this.deleteTableRows(cellsDom);
    this.addPositionableRow(cellsDom, this.columns.length);

    this.eachCellRow(function(i, row){
      this.createCells(i, row);
    }, this);
  },
  setRowSet: function(data){
    var rows = data.rows, cells = data.cells, val;
    var i, n = Math.max(rows.length, cells.length);

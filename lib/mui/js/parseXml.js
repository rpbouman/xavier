...
function unescapeEntities(text) {
  ...
  while (match = re.exec(text)) {
    ...
    if (value = match[3]){
      t += String.fromCharCode(pHex(value));
    }
    else
    if (value = match[4]){
      switch (value) {
...
function serializeToXml(node, indentString, indent) {
  ...
  switch (node.nodeType) {
    case 1:   //element
      ...
      if (
        ...
        str += ">";
        var prefix;
        var i, childNodes = node.childNodes, n = childNodes ? childNodes.length : 0;
        if (n === 1 && childNodes[0].nodeType === 3) {
          str += childNodes[0].data;
          prefix = "";
        }
        else {
          for (i = 0; i < n; i++) {
            str += serializeToXml(childNodes[i], indentString, indent + indentString);
          }
          prefix = "\n" + indent;
        }
    ...
    case 7:   //processing instruction
      str += "\n" + indent + "<?" + node.target + " " + node.content + "?>";
      break;
    case 8:   //comment
      str += "\n" + indent + "<!--" + node.data + "-->";
      break;
    case 9:   //document
      var childNodes = node.childNodes, n = childNodes.length, i, childNode;
      for (i = 0; i < n; i++) {
        childNode = childNodes[i];
        str += serializeToXml(childNode, indentString, indent);
      }
      break;

...
(TabPane = function(conf) {
  ...
  eachTab: function(callback, scope){
    var tab, i, n = this.getTabCount();
    for (i = 0; i < n; i++) {
      tab = this.getTab(i);
      if (callback.call(scope || this, tab, i) === false) {
        return false;
      }
    }
    return true;
...
//          "mantle-default-tab-background",
  ...
  closeTab: function(index) {
    ...
    var componentId = tab.componentId;
    if (componentId) {
      //check if the component is used by other tabs (strange, but possible).
      if (this.eachTab(function(tab, i){

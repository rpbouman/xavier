...
*/
var Observable;
(Observable = function(conf) {
  if (conf && conf.listeners) {
  ...
  fireEvents: function(flag){
    if (arguments.length) {
      if (flag === false) {
        this.dontFireEvents = true;
      }
      else {
        delete this.dontFireEvents;
      }
    }
    else {
      if (iDef(this.dontFireEvents) && this.dontFireEvents === true) {
        return false;
      }
      else {
        return true;

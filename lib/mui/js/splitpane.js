...
(SplitPane = function(conf){
  ...
  setSplitterPosition: function(position) {
    ...
    if (this.fireEvent("beforeSplitterPositionChange", data) === false) {
      return;
    }
...
//    clientProperty = "client" + complementaryPropertyPostfix;
    clientProperty = "offset" + complementaryPropertyPostfix;
    var size = dom[clientProperty];
    var splitterSize = splitter[clientProperty];

    var style;
    style = me.getFirstComponent().style;
    style[complementaryProperty] = position +"px";

    style = me.getSecondComponent().style;
    style[complementaryProperty] = (size - (position + splitterSize)) + "px";
    style[property] = (position + splitterSize) + "px";
  ...
  setOrientation: function(orientation){
    ...
    if (this.fireEvent("beforeOrientationChange", data) === false) {
      return;
    }
  ...
  createDom: function(){
    ...
    if (parentNode = firstComponent.parentNode) {
      parentNode.removeChild(firstComponent);
    }
    firstComponent.className += " " + componentPrefix + " " + componentPrefix + "-first";

    var secondComponent = this.getSecondComponent();
    if (parentNode =  secondComponent.parentNode) {
      parentNode.removeChild(secondComponent);
    }
    ...
    if(conf.splitterPosition) {
      this.setSplitterPosition(conf.splitterPosition);
    }
    return el;
  },
...
SplitPane.clobber = cEl("DIV", {
  id: SplitPane.prefix + "-clobber"
}, null, body);
SplitPane.instances = {};
...
SplitPane.listenToDnd = function(dnd){
  dnd.listen({
    startDrag: function(event, ddHandler) {
      var target = event.getTarget();
      if (!hCls(target, "splitpane-splitter")) {
        return false;
      }
      ...
      switch (orientation) {
        case SplitPane.orientations.horizontal:
          ...
          if (firstSplitPane) min = pDec(firstSplitPane.minHeight);
          currentSplitPane._extra = xy.y - splitterPos.top;
          break;
        case SplitPane.orientations.vertical:
          max = dom.clientWidth - splitter.clientWidth;
          if (firstSplitPane) min = pDec(firstSplitPane.minWidth);
      ...
      var clobber = SplitPane.clobber;
      if (clobber.parentNode) {
        dEl(clobber);
      }
      dom.appendChild(SplitPane.clobber);
      var p = pos(dom);
      var s = clobber.style;
      s.left = p.left + "px";
      s.top = p.top + "px";

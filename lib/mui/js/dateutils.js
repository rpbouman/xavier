...
function isoDateTimeString(date){
  if (!date) {
    date = new Date();
  }
  var fld;
  var isoDateString = date.getFullYear();

  fld = String(date.getMonth() + 1);
  if (fld.length === 1) {
    fld = "0" + fld;
  }
  isoDateString += "-" + fld;

  fld = String(date.getDate());
  if (fld.length === 1) {
    fld = "0" + fld;
  }
  isoDateString += "-" + fld;

  fld = String(date.getHours());
  if (fld.length === 1) {
    fld = "0" + fld;
  }
  isoDateString += "T" + fld;

  fld = String(date.getMinutes());
  if (fld.length === 1) {
    fld = "0" + fld;
  }
  isoDateString += ":" + fld;
